...
function show(io::IO, sym::Union{MachOSymtabEntry,MachOSymbolRef})
    is64bit = isa(deref(sym), nlist_64)
    print(io, "MachOSymbol", is64bit ? " (64 bit)": "")
...
function show(io::IO, syms::MachOSymbols)
    print(io, "MachO Symbol Table")
    for s in syms
        print(io, "\n  ")
        showcompact(io, s)
    end
